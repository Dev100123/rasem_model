...
# ──────────────────
from config_segfomer import IMAGE_SIZE, MEAN, STD
...
# ───────── Config ─────────
from config_segfomer import IMAGE_SIZE, MEAN, STD  # (alto, ancho), tuplas
...
WEIGHTS    = r"../Modelos_Consolidacion_best/modelo_segformer_best_3.pth"
OUT_DIR    = r"Test_SegFormer_Consolidado_"+cultivo
...
def run_test(images_dir: str, masks_dir: str, weights_path: str, out_dir: str):
    ...
    for b in ld:
        ...
        if out.shape[-2:] != gt_r.shape[-2:]:
            out = torch.nn.functional.interpolate(
                out, size=gt_r.shape[-2:], mode="bilinear", align_corners=False
            )

        prob = torch.sigmoid(out)[:, 0]              # (1,H,W)
    ...
    # CSV
    csv_path = os.path.join(out_dir, "metricas_test_segformer_"+cultivo+".csv")
...
def parse_args():
    ...
    ap.add_argument("--weights",default=WEIGHTS,    help="Ruta a pesos .pth (num_labels=1)")
