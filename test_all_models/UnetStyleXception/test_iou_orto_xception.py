...
WEIGHTS    = r"../Modelos_Consolidacion_best/modelo_unetx_best_4.pth"
OUT_DIR    = r"Test_UNetXception_Consolidado_"+cultivo

# ─────────────────────────── PROJECT IMPORTS ───────────────────────────────
#import config
from config_xception import IMAGE_SIZE, DEVICE
CV2_SIZE = (IMAGE_SIZE[1], IMAGE_SIZE[0])

try:
    from train_Xception import UNetXception
except Exception:
    try:
        from train_Xception import UNetXception
    except Exception as e:
        raise ImportError(
            "Could not import UNetXception from 'train_Xception' or 'train'. "
            "Check the name of the file where the model is defined."
        ) from e
...
class PairDataset(Dataset):
    def __init__(self, images_dir, masks_dir):
        ...
        for ip in self.img_files:
            ...
            if mp is None:
                raise RuntimeError(f"No mask was found for the image:\n  {ip}\n"
    ...
    def __getitem__(self, idx):
        ...
        x = (img_res.astype(np.float32) / 255.0).transpose(2, 0, 1)
...
def run_test(images_dir, masks_dir, weights_path, out_dir):
    ...
    # Model
    model = UNetXception(n_classes=1).to(device)
    ...
    # CSV
    csv_path = os.path.join(out_dir, "metricas_test_xception_"+cultivo+".csv")
