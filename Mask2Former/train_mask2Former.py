...
class NopalDataset(Dataset):
    ...
    def _photo_aug(self, img, p=0.3, b_lim=0.2, c_lim=0.2):
        ...
        b = 1.0 + random.uniform(-b_lim, b_lim)
    ...
    def __getitem__(self, idx):
        ...
        img_t = self.norm(img_t)         # ImageNet (0–1)
...
def compute_binary_logit_from_outputs(outputs):
    ...
    # class_logits: [B, Q, C]  (para binario, C≈2 o 1+no_objeto)
    # masks_logits: [B, Q, h, w]
    B, Q, *_ = masks_logits.shape

    # Determinar índice de clase "foreground".
    # Caso 1: C==2 (fondo/foreground). Caso 2: C>2 incluyendo "no-object".
    C = class_logits.shape[-1]
    # Usar softmax sobre clases:
    cls_prob = torch.softmax(class_logits, dim=-1)  # [B,Q,C]

    if C == 1:
        # Si solo hay una clase, tratamos esa como foreground con prob=1
        weights = torch.ones((B, Q, 1), device=masks_logits.device, dtype=masks_logits.dtype)
    else:
        # Heurística: foreground = índice 1 si C==2; si C>2, tomar la mayor prob excepto "no-object" si existe.
        # Si el checkpoint incluye "no-object", suele ir al final; evitamos el último índice.
        if C == 2:
            fg_idx = 1
            weights = cls_prob[:, :, fg_idx:fg_idx+1]  # [B,Q,1]
        else:
            # Excluir último índice como 'no-object'
            weights = cls_prob[:, :, :-1].max(dim=-1, keepdim=True).values  # [B,Q,1]

    # Combinar: suma ponderada de logits de máscara por probabilidad de foreground
    masks = masks_logits  # [B,Q,h,w]
    weights = weights.transpose(1, 2)  # [B,1,Q]
    # Expandir pesos a espacial
...
def main():
    ...
    ignore_modules = [nn.MultiheadAttention]  # Mask2Former uses custom attention modules
